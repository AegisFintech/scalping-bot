  ...
  const rows = await pool.query<{
    ...
    `SELECT ar.analysis_time AS created,min(o.submitted_at) AS available,og.expires_at AS expires,
      ...
      (SELECT m.payload_redacted->>'server_time' FROM model_requests m
       WHERE m.analysis_id=ar.id AND m.context_plan_id=og.context_plan_id
       ORDER BY m.requested_at DESC LIMIT 1) AS decision_at,
     ...
     LEFT JOIN scenario_contexts c ON c.id=og.context_plan_id
     WHERE sv.version=$1 AND a.provider_account_key_hash=$2 AND s.name=$3 AND og.mode='demo'
       AND o.submitted_at IS NOT NULL
     GROUP BY og.id,ar.id,c.id ORDER BY ar.analysis_time LIMIT 10000`,
    [
      release,
      createHash("sha256")
