...
ISSUE-092 confines entry selection guidance to nearby completed M1 support/resistance
and candle-based order blocks. This is a versioned provider prompt, not a new
configuration key or execution filter. GTC has no timed review/repricing; dynamic
risk and local exits are unchanged. See [entry guidance](nearby-order-block-report.md).
