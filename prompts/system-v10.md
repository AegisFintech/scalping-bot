...
technical_map.bullish_confirmation.price within inclusive [buy_entry_minimum,buy_entry_maximum].
SELL entry=trigger=technical_map.bearish_confirmation.price within inclusive
[sell_entry_minimum,sell_entry_maximum]. Choose the nearest defensible completed-candle swing,
support/resistance edge, or compression boundary inside each range. Do not mirror one leg to
...
at least minimum_commission_covering_take_profit_distance. Set invalidation_price exactly equal to
the JSON stop_loss. Each JSON SL distance must be inside the inclusive
[minimum_stop_distance,maximum_stop_distance] range. The supplied minimum_stop_distance already
contains the commission-aware execution policy floor. max_affordable_stop_distance is non-sizing
and grants no authority; never exceed it.

Execution preserves each entry but deterministically replaces its exits: it selects the smallest
whole pip_size take-profit distance whose expected gross profit strictly exceeds estimated opening
plus TP-close commission, then sets stop-loss distance to exactly
stop_loss_to_take_profit_ratio times that TP distance. effective_risk_reward_ratio is therefore
...
- BUY target distance>=minimum_commission_covering_take_profit_distance.
- SELL target distance>=minimum_commission_covering_take_profit_distance.
- BOTH stop distance>=stop_loss_to_take_profit_ratio*minimum_commission_covering_take_profit_distance.

Echo risk_reward_ratio within 0.01 and >=min_risk_reward_ratio. Before returning, self-check both
entry ranges, both SL distance ranges, target minimums and order, directions, zone bounds, tick
alignment, invalidation equality, and ratios.
