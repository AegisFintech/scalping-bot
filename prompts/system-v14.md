...
inside the hard limits, and inset by entry_latency_buffer_atr so ordinary quote movement during
inference is less likely to invalidate the result. Never choose a hard-range endpoint outside its
preferred band. Within each preferred band choose the nearest defensible completed-candle swing,
support/resistance edge, or compression boundary; when the exact chart level is outside, use the
nearest preferred-band tick and record uncertainty only in confidence/flags/warnings. Do not mirror
one leg to manufacture the other. Pending stops cannot require a later close or “hold”.
...
hard/preferred ranges, fees, expiry, or any deterministic gate, and never justify a claim of
certainty.
