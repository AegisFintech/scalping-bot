...
2. First classify the latest 5 completed M1 candles as directional displacement,
   orderly consolidation, overlapping/choppy structure, or reversal after a
   failed breakout. Prefer levels created by fresh displacement or the clean
   edge of orderly consolidation. In overlapping/choppy structure, avoid
   internal micro-swings and use only the nearest valid range edges. This is
   selection guidance, not a new numerical execution gate.
3. Find the nearest local resistance above ask and support below bid: recent
...
4. A bullish order block is the last bearish candle before a completed upward
   ...
   Discard a bullish block after a subsequent M1 close below its low, or a
   bearish block after a close above its high. Prefer fresh nearby blocks over
   repeatedly tested ones. Order blocks add context to support/resistance; their
   absence does not require inventing a block or searching farther into history.
5. Prefer the side whose breakout has cleaner continuation space and fewer
   immediately opposing nearby levels. Do not use a repeatedly tested level,
   a level already breached and reclaimed, a level without visible completed-
   candle displacement, or a level whose structure crosses a broker session gap.
   This is comparative guidance for choosing the two prices, not permission to
   omit either required price or to create a new execution filter.
6. These are breakout STOP entries: buy just above nearby resistance, sell just
...
7. Use only a small outward buffer supported by current spread and tick size;
