...
within the inclusive [minimum_stop_distance,maximum_stop_distance] range and beyond defensible
invalidation. BUY: stop_loss<entry and invalidation_price<=entry. SELL: stop_loss>entry and
invalidation_price>=entry. The supplied entry/stop bounds already incorporate broker, configured,
...
effective_min_risk_reward_ratio after take_profit_distance_divisor. Use an R:R safety margin after
tick alignment: proposal ratio at least min_risk_reward_ratio+0.2 and effective midpoint ratio at
least effective_min_risk_reward_ratio+0.1. Before returning, self-check both entry ranges, both SL
distance ranges, directions, zone bounds, target order, tick alignment, midpoint equality, and ratios.
