...
describe("PM2 deployment configuration", () => {
  ...
  it("contains no trading enablement or credentials", () => {
    ...
    expect(
      config.apps.every(
        (app) =>
          app.env.STRATEGY_VERSION === "0.1.0-actionable-oco-auto-demo.28",
      ),
    ).toBe(true);
    expect(
      config.apps.every(
        (app) => app.env.CODE_VERSION === "0.1.0-actionable-oco-auto-demo.28",
      ),
    ).toBe(true);
    expect(
      config.apps.every(
        (app) =>
          app.env.AUTOMATIC_ANALYSIS_COMPLETED_BASELINE === "2" &&
