...
describe("PM2 deployment configuration", () => {
  ...
  it("contains no trading enablement or credentials", () => {
    ...
    expect(
      config.apps.every(
        (app) =>
          app.env.STRATEGY_VERSION === "0.1.0-actionable-oco-auto-demo.41",
      ),
    ).toBe(true);
    expect(
      config.apps.every(
        (app) => app.env.CODE_VERSION === "0.1.0-actionable-oco-auto-demo.41",
      ),
    ).toBe(true);
    expect(
      config.apps.every(
        (app) =>
          ...
          app.env.PREFERRED_ORDER_EXPIRY_SECONDS === "900" &&
