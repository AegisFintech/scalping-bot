...
describe("concise environment and read-only preflight", () => {
  it("keeps the template concise, stopped and explicit about the exact model", () => {
    ...
    expect(Object.keys(template)).toHaveLength(22);
  ...
  it("distinguishes policy compatibility from the missing demo capital floor", () => {
    const content =
    ...
    expect(JSON.parse(result.output)).toMatchObject({
      valid: false,
      reason: "CONFIG_DEMO_EQUITY_FLOOR_REQUIRED",
    });
    expect(result.unchanged).toBe(true);
  });
