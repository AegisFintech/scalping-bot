...
### ISSUE-084 — Recover incomplete OCO pairs (implemented; validated; delivery pending)
...
- Dependencies: ISSUE-054 campaign controls, ISSUE-057 prompt/bounds trail,
  ...
  demo-only authority.
- Current status: implementation and pre-deployment verification are complete on
...
### ISSUE-065 delivery details

- Acceptance criteria: preserve every schema, semantic, spread, freshness,
  reconciliation, ownership, sizing, margin, precision, emergency, and
  demo-only gate; provide tick-aligned latency-resistant preferred entry bands
  strictly inside the unchanged executable limits; reduce the preferred order
  lifetime only when completed-candle evidence retains observed fills; retain
  an allowlisted aggregate reason for a certainly mapped all-cancelled,
  zero-filled strategy group without guessing the broker's cause; expose that
  evidence to operators; run a commission-inclusive, distinctly labelled
  conservative exit replay; and deploy a new immutable demo release only after
  the full completion gate and a certain broker boundary.
- Dependencies: ISSUE-057 entry bounds, ISSUE-058 broker-minute scheduling,
  ISSUE-061 through ISSUE-063 fee/slippage exits, ISSUE-064 continuous counters,
  and the existing demo-only authority.
- Current status: implementation is in progress on
  `issue-065-demo-conversion-cadence`, tracked by
  [issue #160](https://github.com/AegisFintech/scalping-bot/issues/160). The
  read-only `.39` baseline found 247 attempts, 136 completed endpoint responses,
  37 placed groups, and 14 closed trades over about 15 hours. Ninety-one of 111
  pre-model rejections were spread-related; 65 of 99 completed-response
  rejections had entry geometry invalidated after inference. The affected calls
  had 23.6-second median latency and 0.75 M1-ATR p90 absolute quote movement,
  while endpoint entries clustered at the hard range edges. Fourteen fills
  occurred within 12.69 minutes of group creation, so a 15-minute boundary
  retained every observed fill whereas ten minutes retained only 10 of 14.
  Four certainly mapped groups ended with both strategy orders cancelled and
  zero fill, but no callback supplied a broker error/cause; that distinction
  must be recorded without inventing one. The 14 closed trades had gross demo
  P/L `2.05`, fees `-3.64`, and fee-inclusive net P/L `-1.59`; these are small
  demo observations, not profitability evidence.
  Candidate `.40` uses prompt `system-v13`, `0.75` M1-ATR preferred-entry
  insets, and a replay-supported 15-minute preferred expiry. The strict
  zero-fill terminal predicate is covered positively and with a mixed-state
  rejection path. A 15-candidate completed-M1 exit replay left eight of 14
  trades unresolved inside their fill minute and found every alternative
  fee-negative on the remaining six, so no TP/SL economic change was selected.
...
### ISSUE-044 delivery details

- Acceptance criteria: render Overview group expiry/update timestamps,
  automatic retry timing, analysis and audit selectors, and every displayed
  table timestamp as `DD Mon YYYY, HH:MM:SS GMT+8`; treat naive database
  datetimes as UTC; display invalid values as unavailable; leave PostgreSQL,
  API contracts, Plotly time axes, and exact persisted AI/audit JSON unchanged;
  cover aware, naive, missing, invalid, and dataframe-column paths; and restart
  only Streamlit after all required gates pass.
